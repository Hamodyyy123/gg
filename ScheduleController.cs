...
namespace centrny.Controllers
{
    ...
    public class ScheduleController : Controller
    {
        ...
        private int? GetCurrentUserRootCode()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine("Getting current user RootCode...");

                var rootCodeClaim = User.FindFirst("RootCode");
                System.Diagnostics.Debug.WriteLine($"RootCode claim: {rootCodeClaim?.Value ?? "NULL"}");

                if (rootCodeClaim != null && int.TryParse(rootCodeClaim.Value, out int rootCode))
                {
                    System.Diagnostics.Debug.WriteLine($"Current user RootCode: {rootCode}");
                    return rootCode;
                }

                System.Diagnostics.Debug.WriteLine($"User {User.Identity?.Name} missing or invalid RootCode claim");
                return null;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error in GetCurrentUserRootCode: {ex.Message}");
                return null;
            }
        }
        ...
        /// <summary>
        /// Checks if current user is a center user (root has isCenter="True")
        /// </summary>
        ...
        /// <summary>
        /// Gets the single center for center users (when isCenter="True")
        /// </summary>
        ...
        private async Task<(int? rootCode, string rootName, bool isCenter, string branchName)> GetUserContext()
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"User Identity: {User.Identity?.Name ?? "NULL"}");
                System.Diagnostics.Debug.WriteLine($"User IsAuthenticated: {User.Identity?.IsAuthenticated ?? false}");

                foreach (var claim in User.Claims)
                {
                    System.Diagnostics.Debug.WriteLine($"Claim: {claim.Type} = {claim.Value}");
                }
                System.Diagnostics.Debug.WriteLine("=== END USER CLAIMS ===");
                ...
                System.Diagnostics.Debug.WriteLine($"Parsed user context: RootCode={rootCode}, RootName={rootName}, IsCenter={isCenter}");

                // Get branch information
                string branchName = "Unknown Branch";
                if (rootCode.HasValue)
                {
                    System.Diagnostics.Debug.WriteLine($"Looking for branch with RootCode {rootCode.Value}");
                    ...
                    if (branch != null)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"Found branch: {branchName}");
                    }
                    else
                    {
                        System.Diagnostics.Debug.WriteLine($"No branch found for RootCode {rootCode.Value}");
                    }
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Error in GetUserContext: {ex.Message}");
        ...
        private IQueryable<Schedule> GetFilteredScheduleQuery()
        {
            ...
            {
                query = query.Where(s => s.RootCode == userRootCode.Value);
                System.Diagnostics.Debug.WriteLine($"Applied strict root filtering for RootCode: {userRootCode.Value}");
            }
            else
            {
                System.Diagnostics.Debug.WriteLine($"User {User.Identity?.Name} has no RootCode - returning empty results");
                query = query.Where(s => false); // Returns empty set
            }

            // Apply includes one by one for better error tracking
            try
            {
                query = query.Include(s => s.RootCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added RootCodeNavigation include");

                query = query.Include(s => s.YearCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added YearCodeNavigation include");

                query = query.Include(s => s.HallCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added HallCodeNavigation include");

                query = query.Include(s => s.TeacherCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added TeacherCodeNavigation include");

                query = query.Include(s => s.SubjectCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added SubjectCodeNavigation include");

                query = query.Include(s => s.CenterCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added CenterCodeNavigation include");

                query = query.Include(s => s.BranchCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added BranchCodeNavigation include");

                query = query.Include(s => s.EduYearCodeNavigation);
                System.Diagnostics.Debug.WriteLine("Added EduYearCodeNavigation include");

                return query;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error adding includes to schedule query: {ex.Message}");
                // Return basic query without includes if there are issues
                return _context.Schedules.Where(s => s.RootCode == userRootCode.Value);
            }
        }
        ...
        /// <summary>
        /// GET: Schedule/GetBranchesForCenterUser - API endpoint to get branches for center users
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetBranchesForCenterUser()
        {
            try
            {
                ...
                // For center users, get the single center first
                var centerCode = await GetSingleCenterForCenterUser();
                if (!centerCode.HasValue)
                {
                    return Json(new { success = false, error = "No center found for your account." });
                }
            ...
            {
                _logger.LogError(ex, "Error getting branches for center user {Username}", User.Identity?.Name);
                return Json(new { success = false, error = "Error retrieving branches." });
            }
        }

        /// <summary>
        /// GET: Schedule/GetBranchesForCenter - API endpoint to get branches for a specific center
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetBranchesForCenter(int centerCode)
        {
            try
            {
                ...
                // Verify the center belongs to user's root
        ...
        /// <summary>
        /// GET: Schedule/GetHallsForBranch - API endpoint to get halls for a specific branch
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetHallsForBranch(int branchCode)
        {
            try
            {
                ...
                // Verify the branch belongs to user's root
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                // Debug output
                System.Diagnostics.Debug.WriteLine($"Starting Schedule Index for user {User.Identity?.Name}");

                var (rootCode, rootName, isCenter, branchName) = await GetUserContext();
                System.Diagnostics.Debug.WriteLine($"User context: RootCode={rootCode}, RootName={rootName}, IsCenter={isCenter}");

                if (!rootCode.HasValue)
                {
                    System.Diagnostics.Debug.WriteLine($"No root code found for user {User.Identity?.Name}");
                ...
                System.Diagnostics.Debug.WriteLine($"Fetching BASIC schedules for RootCode {rootCode}");
                ...
                try
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"SUCCESS: Found {schedules.Count} basic schedules");
                }
                catch (Exception basicEx)
                {
                    System.Diagnostics.Debug.WriteLine($"ERROR: Basic query failed: {basicEx.Message}");
                    throw new Exception($"Basic database query failed: {basicEx.Message}", basicEx);
                }

                // Try to add navigation properties one by one
                try
                {
                    System.Diagnostics.Debug.WriteLine("Attempting to reload with navigation properties...");
                    schedules = await _context.Schedules
                        .Where(s => s.RootCode == rootCode.Value)
                        .Include(s => s.RootCodeNavigation)
                        .ToListAsync();
                    System.Diagnostics.Debug.WriteLine("SUCCESS: Added RootCodeNavigation");
                }
                catch (Exception navEx)
                {
                    System.Diagnostics.Debug.WriteLine($"WARNING: Navigation properties failed: {navEx.Message}");
                    // Keep the basic schedules
                }
                ...
                // Get single center code for center users (simplified)
                if (IsCurrentUserCenter())
                {
                    try
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"Center user single center code: {centerCode}");
                    }
                    catch (Exception centerEx)
                    {
                        System.Diagnostics.Debug.WriteLine($"Error getting single center: {centerEx.Message}");
                        ViewBag.SingleCenterCode = null;
                    }
                }

                System.Diagnostics.Debug.WriteLine($"Successfully returning {schedules.Count} schedules for user {User.Identity?.Name}");

                return View(schedules);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"FINAL ERROR in Schedule Index: {ex.Message}");
        ...
        public async Task<IActionResult> Calendar()
        {
            ...
            // Get single center code for center users
            if (IsCurrentUserCenter())
            {
        ...
        public async Task<IActionResult> CreateScheduleEvent([FromBody] ScheduleEventModel model)
        {
            try
            {
                ...
                // Auto-assign center code for center users
                if (IsCurrentUserCenter())
                {
                    var centerCode = await GetSingleCenterForCenterUser();
                    if (centerCode.HasValue)
                    {
                        ...
                        _logger.LogDebug("Auto-setting CenterCode to {CenterCode} for center user {Username}",
                            centerCode.Value, User.Identity?.Name);
                    }
                }
                ...
                var schedule = new Schedule
                {
                    ...
                    CenterCode = model.CenterCode, // Auto-set for center users
                ...
                {
                    var teacherExists = await _context.Teachers.AnyAsync(t => t.RootCode == userRootCode.Value);
                    if (teacherExists)
                    {
        ...
        public async Task<IActionResult> EditScheduleEvent(int id, [FromBody] JsonElement json)
        {
            try
            {
                ...
                // Handle center code - for center users, auto-set; for others, validate
                if (IsCurrentUserCenter())
                {
                    // For center users, maintain their single center
                    var centerCode = await GetSingleCenterForCenterUser();
                    if (centerCode.HasValue)
                    {
                ...
                {
                    // For non-center users, validate center selection
                ...
                // Update branch codes with validation
        ...
        private async Task PopulateDropDownsSafe(Schedule schedule = null)
        {
            ...
            try
            {
                ...
                // Halls - will be loaded dynamically via AJAX
                ViewData["HallCode"] = new SelectList(new List<dynamic>(), "Value", "Text");
                ...
                // Centers - handle differently for center users vs others
                if (IsCurrentUserCenter())
                {
                    // For center users, don't populate centers (will be hidden)
                    ViewData["CenterCode"] = new SelectList(new List<dynamic>(), "Value", "Text");

                    // Get the single center code for ViewBag
                    var singleCenterCode = await GetSingleCenterForCenterUser();
                    ViewBag.SingleCenterCode = singleCenterCode;
                }
                else
                {
                    // For non-center users, show all centers in their root
                ...
                // Branches - will be loaded dynamically via AJAX
                ViewData["BranchCode"] = new SelectList(new List<dynamic>(), "Value", "Text");
    ...
    public class ScheduleEventModel
    {
        ...
        public int? CenterCode { get; set; } // This will be auto-set for center users
